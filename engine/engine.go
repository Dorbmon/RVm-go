...
func (this *RVM) CreateProgress(Name string) (Ok bool, ProgressId uint64) {
	...
	//初始化指令链接系统，并进行初始化链接
	this.ProgressById[ProgressId].Memory = memory.New()
	this.ProgressById[ProgressId].OrderLinker = orderLinker.New()
	this.ProgressById[ProgressId].Stack = stack.New()
	this.ProgressById[ProgressId].Abilities = make(map[string]interface{})
	ability.LoadFunction(rVMToStruct(this), this.ProgressById[ProgressId])
...
func rVMToStruct(RVM *RVM) *StructData.RVM {
	temp := &StructData.RVM{}
	temp.Memory = RVM.Memory
	temp.ProgressById = RVM.ProgressById
	temp.Progress = RVM.Progress
	temp.RunCode = RVM.RunCode
	temp.DebugFilePath = RVM.DebugFilePath
	temp.MainRegister = RVM.MainRegister
	temp.OutputWriter = RVM.OutputWriter
	return temp
}
